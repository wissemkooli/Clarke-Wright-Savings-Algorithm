...
from backend.domain.variants import MDVRPProblem, CVRPProblem
from backend.schemas.vrp import Node, VRPResponse
from backend.schemas.mdvrp import MDVRPResponse, DepotRoutes
from backend.services.osrm import get_osrm_table
from backend.services.utils import haversine_distance, process_osrm_table

def assign_customers(customers: List[Node], depots: List[Node]) -> Dict[int, List[Node]]:
...
def _cw_cost_fast(depot: Node, customers: List[Node], capacity: float, road_dist: Dict[Tuple[int, int], float]) -> float:
    if not customers:
        return 0.0
...
def local_search(assignment: Dict[int, List[Node]], depot_map: Dict[int, Node], capacity: float, road_dist: Dict[Tuple[int, int], float], max_iter: int = 50) -> Tuple[Dict[int, List[Node]], List[str]]:
    steps = []
    for iteration in range(max_iter):
        ...
        for depot_id, customers in list(assignment.items()):
            for customer in list(customers):
                ...
                current_cost = _cw_cost_fast(depot, assignment[depot_id], capacity, road_dist)
                best_gain = 0.0
                best_target = None
...
@SolverRegistry.register("clarke-wright-mdvrp")
class ClarkeWrightMDVRPSolver(BaseSolver):
    name = "Clarke-Wright-MDVRP"
    supported_variants = ["mdvrp"]

    async def solve(self, problem: BaseProblem, **kwargs) -> MDVRPResponse:
        if not isinstance(problem, MDVRPProblem):
            raise ValueError("ClarkeWrightMDVRPSolver only supports MDVRPProblem")

        start_time = time.perf_counter()
        all_nodes = problem.nodes
        depot_ids = problem.depot_ids
        ...
        async def run_cw_for_depot(depot_id: int) -> DepotRoutes:
            ...
            if not depot_customers:
                return DepotRoutes(
                    depot_id=depot_id, routes=[], total_distance=0.0, num_vehicles=0,
                )

            subset_nodes = [depot] + depot_customers
            sub_table = await get_osrm_table([(n.lat, n.lng) for n in subset_nodes])
            
            sub_problem = CVRPProblem(
                nodes=subset_nodes,
                depot_id=depot_id,
                vehicle_capacity=capacity
            )
            result = await cw_solver.solve(sub_problem, precomputed_table=sub_table)
